...
export function Sidebar({ currentPage, onPageChange, isDarkMode, onToggleDarkMode }: SidebarProps) {
  ...
  const navigationItems = [
    ...
    },
    {
      ...
      icon: LineChart,
      description: "NASA data trends"
    },
    {
      id: "simulation-lab",
      name: "Simulation Lab",
      icon: Orbit,
      description: "Digital twin replay"
    },
    {
      id: "engine-details",
      name: "Engine Details",
      icon: Cpu,
      description: "Single-engine deep dive"
    },
    {
      id: "alerts",
      name: "Alerts",
      icon: Siren,
      description: "Alert triage workflow"
    }
  ];
  ...
  return (
    <div className="ml-6 my-6">
      ...
      >
        ...
        <div className="px-4 pb-2 border-t border-sidebar-border/30 pt-4">
          <button
            onClick={onToggleDarkMode}
            className={cn(
              "w-full rounded-xl bg-sidebar-accent/70 hover:bg-sidebar-accent transition-all duration-200",
              "flex items-center justify-center gap-2 text-sidebar-foreground py-2.5",
              !isExpanded && "w-12 h-12 mx-auto rounded-full"
            )}
        ...
        <div className="px-4 pb-4 pt-2 flex justify-center">
          <div className={cn("relative group", isExpanded && "w-full") }>
            <div 
              className={cn(
                "w-12 h-12 bg-gradient-to-br from-sidebar-primary to-sidebar-primary/80 rounded-full flex items-center justify-center shadow-lg hover:scale-110 transition-all duration-300 cursor-pointer",
                isExpanded && "mx-auto"
              )}
              onClick={() => setIsExpanded(!isExpanded)}
            >
              <span className="text-sidebar-primary-foreground font-semibold text-lg">LA</span>
            </div>
            
            {/* Profile tooltip for collapsed state */}
            {!isExpanded && (
              <div className="absolute left-full ml-4 px-3 py-2 bg-gradient-to-br from-sidebar-primary to-sidebar-primary/90 text-sidebar-primary-foreground rounded-xl opacity-0 group-hover:opacity-100 transition-all duration-300 pointer-events-none whitespace-nowrap z-50 shadow-lg transform translate-x-2 group-hover:translate-x-0">
                <div className="font-medium text-sm">Lakshay</div>
